...
with open('simulations.txt','w') as positions:
    positions.write('body;x-dir;y-dir;z-dir\n')
...
def advance(dt, n, bodies=SYSTEM, pairs=PAIRS):
    with open('simulations.txt','a') as positions:
        for i in range(n):
            ...
            for (r, [vx, vy, vz], m) in bodies:
                ...
                r[1] += dt * vy
                r[2] += dt * vz
                #bodies_values = list(bodies)
                bodies_keys = list(BODIES.keys())
                index = bodies.index((r, [vx, vy, vz], m))
                object_ = bodies_keys[index]
                x_coord = r[0]
                y_coord = r[1]
                z_coord = r[2]
                sim_to_file = "{};{};{};{}\n".format(object_,x_coord,y_coord,z_coord)
                positions.write(sim_to_file)
