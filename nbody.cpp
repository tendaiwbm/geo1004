...
void advance(body state[BODIES_COUNT], double dt) {
    ...
    for (unsigned int i = 0; i < BODIES_COUNT; ++i) {
        ...
        std::fstream particles_output{ "D:\\nbody\\particles.txt", std::ios::app };
        particles_output << state[i].name << ';' << state[i].position.x << ';' << state[i].position.y << ';' << state[i].position.z << '\n';
...
int main(int argc, char **argv) {
    if (argc != 2) {
        ...
        std::fstream particles_output{ "D:\\nbody\\particles.txt", std::ios::out };
        particles_output << "body;x-dir;y-dir;z-dir" << '\n';
        particles_output.close();
